...
- Refresh button (optional, low priority)
...
- Update `tierFor()` function to use new thresholds:
  - Green: < 70%
  - Amber: ≥ 70% and < 90%
  - Red: ≥ 90%
- This ensures icon and popover bar use identical colour logic
...
2. ✓ Colour changes appropriately (green → amber → red)
3. ✓ Clicking icon opens popover with bar + percentage + reset time
4. ✓ 7-day quota removed from display entirely
5. ✓ Error handling preserved (API failures show warning banner)
6. ✓ Single instance enforcement maintained
...
- **Popover size:** Can reduce from 290×240 to ~200×100 since we're only showing one bar. Keep flexible for future dashboard expansion.
